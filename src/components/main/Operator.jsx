...
const Operator = () => {
  ...
  const sortedTickets = [...submittedTickets].sort((a, b) => {
    if (sortBy === "numbers") {
      const numbersA = a.numbers.join(",");
      const numbersB = b.numbers.join(",");
      return sortOrder === "asc"
        ? numbersA.localeCompare(numbersB)
        : numbersB.localeCompare(numbersA);
      ...
      const matchesB = calculateMatches(b.numbers, generateRandomNumbers())
        .matchingCounts[5];
      return sortOrder === "asc" ? matchesA - matchesB : matchesB - matchesA;
    } else if (sortBy === "payout") {
      const payoutA = calculateMatches(
        a.numbers,
        generateRandomNumbers()
      ).totalPayout;
      const payoutB = calculateMatches(
        b.numbers,
        generateRandomNumbers()
      ).totalPayout;
  ...
  return (
    <div className="container mx-auto my-8 p-6 bg-gray-200 rounded-md">
      ...
      <div className="mt-8">
        ...
        <button
          onClick={resetRound}
          className="ml-4 bg-gray-500 text-white px-4 py-2 rounded-md hover:bg-gray-700 focus:outline-none focus:ring focus:border-gray-300"
        >
          Reset Round
        </button>
        <button
